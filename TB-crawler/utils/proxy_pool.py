...
from storage.db_handler import  MySQLProxyClient
import urllib3
import os
...
class ProxyPool:
    ...
    def _get_valid_proxies(self):
        ...
        if not proxies:
            ...
            proxies = self.db_client.get_valid_proxies()  # 重新获取刷新后的代理
        logger.info(f"从数据库加载有效代理数量: {len(proxies)}")
        return proxies
    def check_proxy_num(self):
        if self.db_client.has_enough_proxies() != True:
            self.refresh_proxies()

        return self.db_client.has_enough_proxies(10)
...
class ProxyIP:
    ...
    def get_ip_port(self):
        ...
        for page in range(self.ip_page):
            try:
                url=f'{self.url}?protocol=&country=&per_page=10&page={page}'
                res = requests.get(
                    url,
